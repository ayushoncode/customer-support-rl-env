...
TASKS = {
    "easy": [
        {"id": "easy_refund", "order_id": "ORD-4521", "email": "Hi, my order #ORD-4521 is delayed by 2 weeks. I'd like a full refund please.", "keywords": {"solution": ["refund", "reimburs"], "empathy": ["sorry", "apologize", "apologi", "understand", "inconvenien"]}},
        {"id": "easy_tracking", "order_id": None, "email": "Hello, I placed an order 5 days ago and haven't received any tracking info. Can you help?", "keywords": {"solution": ["track", "shipment", "dispatch", "update", "status"], "empathy": ["sorry", "apologize", "apologi", "understand", "inconvenien"]}},
    ],
    "medium": [
        ...
        {"id": "medium_billing", "order_id": "ORD-6612", "email": "I was charged twice for the same order. I want my money back NOW. This is ridiculous.", "keywords": {"solution": ["refund", "revers", "reimburse", "charge", "credit"], "empathy": ["sorry", "apologize", "apologi", "understand", "inconvenien"]}},
    ],
    "hard": [
        {"id": "hard_missing_laptop", "order_id": "ORD-9921", "email": "I am absolutely furious! My $1200 laptop has been missing for 3 weeks. I have called THREE times. Fix this NOW or I am calling my lawyer!", "keywords": {"solution": ["escalat", "investigat", "replac", "refund", "manag", "compensat", "priorit"], "empathy": ["sorry", "apologize", "apologi", "understand", "frustrat", "sincer", "concern"]}},
        {"id": "hard_fraud", "order_id": None, "email": "Someone used my account to make unauthorized purchases totaling $800. I've been a customer for 5 years! I'm disputing with my bank!", "keywords": {"solution": ["secur", "block", "refund", "fraud", "investigat", "escalat", "account"], "empathy": ["sorry", "apologize", "apologi", "understand", "frustrat", "sincer", "concern", "valued"]}},
    ],
}
...
class SupportEnv:
    def __init__(self):
        ...
        self.frustration_meter: float = 0.0
        self.steps_taken: int = 0
        self.researched: bool = False
    ...
    def reset(self, difficulty: str = "easy") -> SupportObservation:
        ...
        self._observation = SupportObservation(
            ...
            policy_snippet=f"Refund window: {POLICY['refund_window_days']} days. Max refund: ${POLICY['max_refund_amount']}. Escalate orders over ${POLICY['escalation_threshold']}.",
        ...
        if action.action_type == "RESEARCH":
            ...
            if not self.drafted:
                ...
                if self.frustration_meter >= 100:
                    reward = -1.0
                    feedback_parts.append("CUSTOMER CHURNED - frustration hit 100%")

                if hallucination_penalty > 0:
                    feedback_parts.append(f"Hallucination penalty: -{hallucination_penalty:.2f}")
                if policy_penalty > 0:
                    feedback_parts.append(f"Policy violation penalty: -{policy_penalty:.2f}")
    ...
    def state(self) -> SupportState:
        if not self.current_task:
            return SupportState(status="idle", feedback="No active task. Call reset() first.", frustration_meter=0.0, steps_taken=0)
        return SupportState(
            status="active",
